...
public class SM4128TsFileReadWriteTest {
  ...
  private File f;
  private final IDeviceID deviceID = new PlainDeviceID("device_1");
  ...
  private void writeDataByTSRecord(
      TSDataType dataType, SM4128TsFileReadWriteTest.DataPointProxy proxy, TSEncoding encodingType)
  ...
  private void readData(SM4128TsFileReadWriteTest.ReadDataPointProxy proxy) throws IOException {
    TsFileSequenceReader reader = new TsFileSequenceReader(path);
    TsFileReader readTsFile = new TsFileReader(reader);
    ArrayList<Path> paths = new ArrayList<>();
